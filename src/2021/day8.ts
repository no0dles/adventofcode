...
export function solution1(lines: Parsed[]): number {
  return lines.reduce((count, line) => {
    return (
      count +
      line.output.filter((o) => uniqueNumbers.indexOf(o.length) >= 0).length
    )
  }, 0)
}
...
export function solution2(lines: Parsed[]): number {
  return lines.reduce((count, line) => {
    ...
    const fourMinusSevenChars = four
      .split('')
      .filter((c) => one.indexOf(c) === -1)
      .join('')

    const nine = sixDigits.find(
      (i) => hasChars(i, seven) && hasChars(i, fourMinusSevenChars)
    )
    ...
    return (
      count +
      line.output.reduce((total, output, index) => {
